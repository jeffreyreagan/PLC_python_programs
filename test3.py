...
class PLCInterface:
    def __init__(self, master):
        ...
        self.diagram_canvas = Canvas(self.diagram_window, width=1200, height=800)
        self.diagram_canvas.pack(expand=YES, fill=BOTH)
    ...
    def update_valve_states(self):
        ...
        self.show_diagram()  # Call the show_diagram method to update the diagram
        self.master.after(self.update_interval, self.update_valve_states)
    ...
    def show_diagram(self):
        self.diagram_canvas.delete("all")  # Clear the canvas
        ...
        self.draw_valves_and_pipes(valve_positions, rto_positions, atmosphere_positions)
        ...
        self.draw_valves_and_pipes(valve_positions_group2, rto_positions_group2, atmosphere_positions_group2)

    def draw_machines(self, machine_positions, label_prefix):
        for i, position in enumerate(machine_positions):
            self.diagram_canvas.create_text(position[0] + 15, position[1] - 10, text=f"{label_prefix} {i + 1}")
            self.diagram_canvas.create_line(position[0] + 15, position[1], position[0] + 15, position[1] + 30, width=5)

    def draw_valves_and_pipes(self, valve_positions, rto_positions, atmosphere_positions):
        for i, (valve_position, rto_position, atmosphere_position) in enumerate(
            zip(valve_positions, rto_positions, atmosphere_positions)
        ):
            ...
            self.diagram_canvas.create_line(valve_position[0] + 15, valve_position[1] + 30, rto_position[0] + 15, rto_position[1], width=5)
            self.diagram_canvas.create_line(
                valve_position[0] + 30, valve_position[1] + 15, atmosphere_position[0], atmosphere_position[1], width=5
            )

            # Label pipes
            self.diagram_canvas.create_text(rto_position[0] + 60, rto_position[1] - 15, text=f"RTO Pipe {i + 1}", anchor=W)
            self.diagram_canvas.create_text(atmosphere_position[0] + 15, atmosphere_position[1], text="Atmosphere", anchor=W)

            # Label valve slightly higher and to the left
            self.diagram_canvas.create_text(valve_position[0] + 5, valve_position[1] + 30, text=f"Valve {i + 1}", anchor=E)

        # Draw RTO pipes connecting to each other
        for i in range(len(rto_positions) - 1):
            self.diagram_canvas.create_line(
                rto_positions[i][0] + 15, rto_positions[i][1], rto_positions[i + 1][0] + 15, rto_positions[i + 1][1], width=5
            )
